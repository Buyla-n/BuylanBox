...
object AXMLPrinter {
    ...
    val START_DOCUMENT: Int = 0
    val END_DOCUMENT: Int = 1
    val START_TAG: Int = 2
    val END_TAG: Int = 3
    val TEXT: Int = 4
            ...
            return buildString {
                ...
                while (true) {
                    ...
                    when (type) {
                        ...
                        START_TAG -> {
                            append("$indent<${getNamespacePrefix(parser.getPrefix())}${parser.getName()}")
                            indent += indentStep

                            // Namespaces
                            val namespaceCountBefore =
                                parser.getNamespaceCount(parser.getDepth() - 1)
                            val namespaceCount = parser.getNamespaceCount(parser.getDepth())
                            for (i in namespaceCountBefore until namespaceCount) {
                                append(
                                    "\n${indent}xmlns:${parser.getNamespacePrefix(i)}=\"${
                            ...
                            for (i in 0 until parser.getAttributeCount()) {
                                append(
                                    "\n$indent${getNamespacePrefix(parser.getAttributePrefix(i))}${
